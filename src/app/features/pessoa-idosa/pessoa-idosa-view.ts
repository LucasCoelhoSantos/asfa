...
export class PessoaIdosaViewComponent implements OnInit, OnDestroy {
  ...
  private carregarPessoaIdosa(id: string) {
    ...
    this.pessoaIdosa$ = this.pessoaIdosaService.obterPorId(id);
    ...
    this.pessoaIdosa$.pipe(takeUntil(this.destroy$)).subscribe({
      next: (pessoa) => {
        if (pessoa) {
          ...
          this.error = true;
          this.notificationService.mostrarErro('Pessoa idosa não encontrada.');
        }
        this.loading = false;
      },
      error: () => {
        this.error = true;
        this.loading = false;
        this.notificationService.mostrarErro('Erro ao carregar dados da pessoa idosa.');
      }
    });
  }
  ...
  async exportarPdf() {
    ...
    try {
      await this.pdfService.gerarPdfPessoa(this.pessoaIdosa!);
      this.notificationService.mostrarSucesso('PDF gerado com sucesso!');
    } catch (error) {
      this.notificationService.mostrarErro('Erro ao gerar PDF.');
      console.error(error);
    }
  }
  ...
  obterCategoriaAnexoLabel(categoria: number | undefined): string {
    if (typeof categoria !== 'number') return 'Documento';
    const encontrada = CATEGORIAS_ANEXO.find(c => c.categoria === categoria);
    return encontrada?.label || 'Documento';
  }

  obterCategoriaAnexoBadgeClass(categoria: number | undefined): string {
    if (typeof categoria !== 'number') return 'bg-secondary';
    const encontrada = CATEGORIAS_ANEXO.find(c => c.categoria === categoria);
    return encontrada?.class || 'bg-secondary';
  }

  inferirTipoArquivo(nomeArquivo?: string, caminho?: string): 'image' | 'pdf' | 'other' {
